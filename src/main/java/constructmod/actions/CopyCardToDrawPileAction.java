...
public class CopyCardToDrawPileAction extends AbstractGameAction
{
    ...
    public CopyCardToDrawPileAction(final AbstractCreature source, final int amount) {
    ...
    public void update() {
        if (this.duration == Settings.ACTION_DUR_FAST) {
            ...
            if (this.p.hand.group.size() > 1) {
                AbstractDungeon.handCardSelectScreen.open(CopyCardToDrawPileAction.TEXT, 1, false, false, false, false);
                this.tickDuration();
                return;
            }
            if (this.p.hand.group.size() == 1) {
